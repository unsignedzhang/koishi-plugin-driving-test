...
exports.apply = exports.Config = exports.name = exports.sleep = void 0;
const koishi_1 = require("koishi");
const axios_1 = __importDefault(require("axios"));
const cross_fetch_1 = __importDefault(require("cross-fetch"));
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
exports.sleep = sleep;
exports.name = 'driving-test';
...
function apply(ctx) {
    ctx.command('驾考答题', '开始驾驶理论考试答题').action(async ({ session }) => {
    ...
    ctx.command('结束驾考答题', '结束驾考答题').action(async (_, message) => {
        if (globalflag != true)
            return "已结束！";
        globalflag = false;
        let rate = bingo / num;
        ...
        if (rate == 1) {
            ...
            if (num >= 10)
                response = "结束成功。本次测试总共" + num + "题，恭喜您全部答对！真不愧是老司机~";
        }
        if (rate >= 0.9)
            response = "结束成功。本次测试总共" + num + "题，您答对" + bingo + "道，考科目一应该是稳了~";
        if (rate >= 0.8)
            response = "结束成功。本次测试总共" + num + "题，您答对" + bingo + "道，考科目一还有点悬，请继续努力哦";
        if (rate >= 0.6)
            response = "结束成功。本次测试总共" + num + "题，您答对" + bingo + "道，还需要继续努力哦~";
        if (rate <= 0.6)
            response = "结束成功。本次测试总共" + num + "题，您答对" + bingo + "道，正确率有点低，多练习一下吧~";
        if (rate == 0)
            response = "结束成功。本次测试总共" + num + "题，您一道都没答对，难道您就是传说中的马路杀手？";
        bingo = 0;
        num = 0;
        return response;
    });
    ctx.middleware(async (session, next) => {
        ...
        if (globalflag != true)
            return next(); //未开启答题，不处理
        if (!isValidCombination(idiom))
            return next(); //回答不是规定的数字或字母，不处理
...
async function getQuestion() {
    ...
    if (responseData.question != "") {
        ...
        response = response + (0, koishi_1.h)("image", { url: responseData.imageurl });
...
async function getRequestWithCookies(url) {
    // 创建一个 Axios 实例  
...
function isEquivalentAnswer(answer, rightAnswer) {
...
function replaceText(text, searchText, replaceText) {
    return text.replace(new RegExp(searchText, 'g'), replaceText);
}
async function sendGetRequest({ url, data, cookies }) {
    try {
        const options = {
            method: 'GET',
            headers: {
...
function isValidCombination(str) {
